# Matéo begins
# This file contains the implementation of the alpha sweep with inlier-outlier model model, and CSV saving at each point. Plots are made and saved at the end.
import numpy as np
...
# Choose what to plot (possible values: "m", "q", "V", "RS", "angle_teacher_student", "m_hat", "q_hat", "V_hat", "excess_gen_error", "estim_error", "time")
plotted_values = ["m", "q", 
...
# CSV Header
if save_CSV:
    header_CSV = "alpha,m,q,V,m_hat,q_hat,V_hat,gen_error,estim_error,angle_teacher_student,rs_value,time_sec\n"
...
for idx, alpha in enumerate(tqdm(alphas, desc="Alpha Sweep")):
    ...
    try:
        ...
        if np.all(np.isfinite([m, q, V])):
            ...
            # Compute angle between teacher and student
            angle_teacher_student_value = angle_teacher_student(m, q, V)
            ...
            # Success
            if np.all(np.isfinite([m, q, V, m_hat, q_hat, V_hat, excess_gen_err, estim_err, rs_value, angle_teacher_student_value])):
                success = True
    ...
    if success:
        ...
        angle_teacher_student_results[idx] = angle_teacher_student_value
        ...
        if save_CSV:
            try:
                with open(file_path_CSV, "a") as f:
                    ...
                    f.write(f"{alpha:.8e},{m:.8e},{q:.8e},{V:.8e},{m_hat:.8e},{q_hat:.8e},{V_hat:.8e},{excess_gen_err:.8e},{estim_err:.8e},{angle_teacher_student_value:.8e},{rs_str},{point_duration:.4f}\n")
                    f.flush()
            except IOError as e:
