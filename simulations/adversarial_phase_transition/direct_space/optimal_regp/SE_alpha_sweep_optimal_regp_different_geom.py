...
def compute_theory_overlaps(reg_param, alpha, init_cond):
    ...
    f_kwargs = {"reg_param": reg_param, "reg_order": reg_p, "pstar": pstar_t}
...
def fun_to_min(reg_param, alpha, init_cond, error_metric="misclass"):
    ...
    if error_metric == "adv":
        return classification_adversarial_error(m_se, q_se, P_se, eps_test, pstar_g)
    elif error_metric == "misclass":
        return misclassification_error_direct_space(m_se, q_se, P_se, eps_test, pstar_g)
    elif error_metric == "bound":
        return boundary_error_direct_space(m_se, q_se, P_se, eps_test, pstar_g)
    else:
        return flipped_error_direct_space(m_se, q_se, P_se, eps_test, pstar_g)


data_folder = "./data/direct_space_model_training_optimal"

file_name_misclass = f"SE_optimal_regp_misclass_direct_alphas_{alpha_min:.1f}_{alpha_max:.1f}_{n_alphas:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
file_name_adverr = f"SE_optimal_regp_adverr_direct_alphas_{alpha_min:.1f}_{alpha_max:.1f}_{n_alphas:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
file_name_bound = f"SE_optimal_regp_bound_direct_alphas_{alpha_min:.1f}_{alpha_max:.1f}_{n_alphas:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
...
def perform_sweep(error_metric_type, output_file):
    ...
    for j, alpha in enumerate(alphas):
        ...
        adversarial_errors_found[j] = classification_adversarial_error(
            ms_found[j], qs_found[j], Ps_found[j], eps_test, pstar_g
        )
