...
file_name_misclass = f"ERM_optimal_regp_misclass_direct_alphas_{alpha_min_erm:.1f}_{alpha_max_erm:.1f}_{n_alphas_erm:d}_delta_{delta:.2f}_d_{d:d}_reps_{reps:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
file_name_adverr = f"ERM_optimal_regp_adverr_direct_alphas_{alpha_min_erm:.1f}_{alpha_max_erm:.1f}_{n_alphas_erm:d}_delta_{delta:.2f}_d_{d:d}_reps_{reps:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
file_name_bound = f"ERM_optimal_regp_bound_direct_alphas_{alpha_min_erm:.1f}_{alpha_max_erm:.1f}_{n_alphas_erm:d}_delta_{delta:.2f}_d_{d:d}_reps_{reps:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"

file_name_misclass_SE = f"SE_optimal_regp_misclass_direct_alphas_{alpha_min_se:.1f}_{alpha_max_se:.1f}_{n_alphas_se:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
file_name_adverr_SE = f"SE_optimal_regp_adverr_direct_alphas_{alpha_min_se:.1f}_{alpha_max_se:.1f}_{n_alphas_se:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
file_name_bound_SE = f"SE_optimal_regp_bound_direct_alphas_{alpha_min_se:.1f}_{alpha_max_se:.1f}_{n_alphas_se:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
...
def perform_sweep(metric_name, file_name_SE_template, file_name_output):
    ...
    for i, (alpha, reg_param) in enumerate(zip(alpha_list, reg_param_list)):
        ...
        while j < reps:
            ...
            P_vals.append(np.sum(np.abs(w) ** pstar_t) / d)
            ...
            adv_perturbation = find_adversarial_perturbation_direct_space_noteacher(
                ys_gen, xs_gen, w, wstar, eps_test_tilde, adv_geometry_g
            )
            ...
            adv_perturbation = find_adversarial_perturbation_direct_space(
                yhat_gen, xs_gen, w, wstar, eps_test_tilde, adv_geometry_g
            )
