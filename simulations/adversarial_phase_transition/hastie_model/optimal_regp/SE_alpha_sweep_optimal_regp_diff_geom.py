...
def fun_to_min(reg_param, alpha, gamma, init_cond, error_metric="misclass"):
    ...
    if error_metric == "adv":
        return classification_adversarial_error_latent(
            m_se, q_se, q_features_se, q_latent_se, 1.0, P_se, eps_test, gamma, pstar_g
        )
    elif error_metric == "misclass":
        return percentage_misclassified_hastie_model(
            m_se, q_se, q_latent_se, q_features_se, 1.0, eps_test, gamma, adv_geometry_g
        )
...
file_name_misclass = f"SE_optimal_regp_misclass_gamma_{gamma:.2f}_alphas_{alpha_min:.1f}_{alpha_max:.1f}_{n_alphas:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
file_name_flipped = f"SE_optimal_regp_flipped_gamma_{gamma:.2f}_alphas_{alpha_min:.1f}_{alpha_max:.1f}_{n_alphas:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
file_name_bound = f"SE_optimal_regp_bound_gamma_{gamma:.2f}_alphas_{alpha_min:.1f}_{alpha_max:.1f}_{n_alphas:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
file_name_adverr = f"SE_optimal_regp_adverr_gamma_{gamma:.2f}_alphas_{alpha_min:.1f}_{alpha_max:.1f}_{n_alphas:d}_pstar_{pstar_t:.1f}_{pstar_g:.1f}_reg_{reg_p:.1f}.csv"
