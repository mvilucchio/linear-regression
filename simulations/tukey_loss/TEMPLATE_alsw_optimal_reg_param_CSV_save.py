...
reg_fun_name = "L2"
min_reg_param = -100000.0 #lambda_min
...
plotted_values = ["m", "q", 
                  "V", "RS", "angle_teacher_student",
                   "m_hat", "q_hat", "V_hat", "opt_reg_param",
                   "excess_gen_error", "estim_error", 
                   "time"]

if save_plots:

    plot_folder = f"./imgs/alpha_sweeps_{loss_fun_name}_{reg_fun_name}_decorrelated_noise_opt_reg_param_for_{f_min.__name__}"

    if not os.path.exists(plot_folder):
        os.makedirs(plot_folder)

    plot_subfolder = f"{plot_folder}/loss_param_{loss_parameters['tau']:.1f}_noise_{noise['Delta_in']:.2f}_{noise['Delta_out']:.2f}_{noise['percentage']:.2f}_{noise['beta']:.2f}"

    if not os.path.exists(plot_subfolder):
        os.makedirs(plot_subfolder)

    figures_folder = f"{plot_subfolder}/alsw_alpha_min_{alpha_min:.1f}_max_{alpha_max:.1f}_n_pts_{n_alpha_pts}_min_reg_param_{min_reg_param:.1f}"
...
if save_CSV or save_pickle:
    data_folder = f"./data/alpha_sweeps_{loss_fun_name}_{reg_fun_name}_decorrelated_noise_opt_reg_param_for_{f_min.__name__}"

    if not os.path.exists(data_folder):
        os.makedirs(data_folder)

    data_subfolder = f"{data_folder}/loss_param_{loss_parameters['tau']:.1f}_noise_{noise['Delta_in']:.2f}_{noise['Delta_out']:.2f}_{noise['percentage']:.2f}_{noise['beta']:.2f}"

    if not os.path.exists(data_subfolder):
        os.makedirs(data_subfolder)

    file_name_base = f"alsw_alpha_min_{alpha_min:.1f}_max_{alpha_max:.1f}_n_pts_{n_alpha_pts}_min_reg_param_{min_reg_param:.1f}"
...
if save_plots:
    ...
    if any(k in plotted_values for k in ["m_hat", "q_hat", "V_hat", "opt_reg_param"]):
        plt.figure()
        ...
        plt.title("m_hat, q_hat, V_hat, opt_reg_param vs alpha")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(figures_folder, "mqVhat_opt_reg_vs_alpha.png"))
        plt.close()
