...
file_name = f"optimal_se_tukey_{alpha_min}_{alpha_max}_{n_alpha_pts}_{delta_in}_{delta_out}_{percentage}_{beta}_{c}.pkl"
...
for d in ds:
    ...
    for i, alpha in enumerate(tqdm(alphas)):
        ...
        for rep in tqdm(range(reps), leave=False):
            ...
            w = find_coefficients_mod_Tukey(
                ys, xs, reg_param_opt[i], hub_params_opt[i], c, p=2, initial_w=w_init
            )
    ...
    file_name_erm = f"optimal_erm_tukey_{alpha_min}_{100}_{len_alphas}_{d}_{delta_in}_{delta_out}_{percentage}_{beta}_{reg_param}_{tau}_{c}.pkl"
