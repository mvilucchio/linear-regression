...
# --- Paramètres pour la visualisation --- Matéo
...
gamma_min, gamma_max = 1.1*tau_vis, PLOT_RANGE_EXTENSION * (DEFAULT_N_STD*np.sqrt(delta_prime_in)) # changer in / out et adapter pour c0
...
for i, (Z, title, cmap) in enumerate(zip(integrands, titles, cmaps)):
    ...
    ax.set_ylim(tau_vis, DEFAULT_N_STD*np.sqrt(delta_prime_in*mu_in )) # changer in /out

    # Ajuster l'angle de vue
    # ax.view_init(elev=20., azim=-65)

fig.suptitle(
    f'Visualisation 3D des Intégrandes inliers (Tukey Mod.thin) pour τ={tau_vis:.2f}, c={C_TUKEY:.3f}, λ={lambda_vis:.2f}, δ_in={DELTA_IN:.2f}, δ_out={DELTA_OUT:.2f}, β={BETA:.2f}, alpha={alpha:.2f}',
    fontsize=11
) #changer in / out
