...
alpha_min, alpha_max, n_alpha_pts = 0.1, 100, 250
reg_orders = [1, 2]
eps_t = 0.3
eps_g = eps_t
reg_param = 1e-2
...
if __name__ == "__main__":
    if run_experiments:
        ...
        for i, reg_order in enumerate(reg_orders):
            ...
            if not os.path.exists(os.path.join(data_folder, file_name.format(reg_order))):
                if reg_order == 1:
                ...
                with open(os.path.join(data_folder, file_name.format(reg_order)), "wb") as f:
                    pickle.dump(data, f)

    plt.figure(figsize=(15, 8))
    for i, reg_order in enumerate(reg_orders):
        with open(os.path.join(data_folder, file_name.format(reg_order)), "rb") as f:
        ...
        if reg_order == 1:
            with open(os.path.join(data_folder, file_name_erm_1.format(reg_order)), "rb") as f:
                data_erm = pickle.load(f)
        else:
            with open(os.path.join(data_folder, file_name_erm.format(reg_order)), "rb") as f:
                data_erm = pickle.load(f)
    ...
    plt.suptitle(
        r"L$\infty$ attack with regularisation $L r$ $\varepsilon_t$ = {:.1e}, $\varepsilon_g$ = {:.1e}".format(
            eps_t, eps_g
        )
    )
