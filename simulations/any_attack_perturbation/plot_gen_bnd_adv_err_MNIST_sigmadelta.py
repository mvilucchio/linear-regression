...
data_folder = "./data"
file_name = f"MNIST_ERM_data_sigmadelta_{sigmadelta[0]:.1f}_{sigmadelta[1]:.1f}_sigmaw_{{:.1f}}_{{:.1f}}_alpha_{alpha_min:.3f}_{alpha_max:.3f}_{n_alpha_pts:d}_digits_{DIGIT_1}vs{DIGIT_2}_reps_{{:d}}_reg_param_{{:.1e}}_eps_t_g_{eps_t:.1e}_{eps_g:.1e}.pkl"
...
for i, sigmaw_spec in enumerate(sigmaws):
    try:
        # Try to load the data
        with open(
            join(data_folder, file_name.format(sigmadelta[0], sigmadelta[1], reps, reg_param)), "rb"
        ) as f:
        ...
        print(gen_error_mean)
        print(adv_error_mean)
        ...
        print(f"File for {sigmaw_spec} not found, skipping.")
    except Exception as e:
        print(f"Error loading {sigmaw_spec}: {e}")
...
plt.savefig(
    join(
        IMGS_FOLDER,
        f"MNIST_sigmadelta_{DIGIT_1}vs{DIGIT_2}_errors_lambda_{reg_param:.0e}_eps_{eps_t:.1e}.pdf",
    ),
    format="pdf",
    dpi=300,
)
