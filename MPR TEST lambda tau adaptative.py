...
# Indexation : [index_reg_param, index_tau, (m=0, q=1, V=2)]
MQV_results = np.full((N_REG_PARAM_PTS, N_TAU_PTS, 3), np.nan)
...
print(f"        {N_TAU_PTS} points tau [{taus[0]:.2f}, {taus[-1]:.2f}] (parcouru de min à max)")

# Boucle externe sur tau (index j, croissant) - avec tqdm en position 0
# Utilisation de `position=0` pour la barre externe principale
for j in tqdm(range(N_TAU_PTS), desc="Progression Tau", position=0):
    tau = taus[j]

    # Initialisation pour le premier reg_param (le plus grand) de cette colonne tau
    if j > 0:
        last_m, last_q, last_V = MQV_results[N_REG_PARAM_PTS - 1, j - 1, :]
    ...
    for i in inner_pbar:
        ...
        try:
            ...
            if np.isfinite(m) and np.isfinite(q) and np.isfinite(V):
                MQV_results[i, j, :] = (m, q, V)
...
print("Extraction des résultats V et génération du graphique...")
...
# Ajout de la ligne de contour pour V = 5/4 (1.25)
try:
    if not np.all(np.isnan(V_results)):
        contour_plot = ax.contour(T, R, V_results, levels=[1.25], colors='red', linestyles='--', linewidths=2)
        ax.clabel(contour_plot, inline=True, fontsize=10, fmt='V=%.2f' % 1.25)
except ValueError as e:
    print(f"Impossible de tracer la ligne de contour V=5/4 : {e}")
except Exception as e:
