...
def find_optimal_reg_param_function_(
    ...
    def minimize_fun(reg_param):
        ...
        if reg_param < min_reg_param:
            if verbose:
                print(f"\t\tλ < min_reg_param ({min_reg_param}), returning barrier penalty")
            return barrier_penalty + 1e3 * (reg_param - min_reg_param)**2
        ...
        if RS_func is not None and barrier_RS_threshold is not None:
            ...
            try:
                ...
                if RS_val >= barrier_RS_threshold:
                    ...
                    return barrier_penalty + 1e3 * (RS_val - barrier_RS_threshold)**2
...
def find_optimal_reg_and_loss_param_function(
    ...
    f_kwargs: dict,
    f_hat_kwargs: dict,
    initial_guess_reg_and_loss_param: tuple[float, float],
    initial_cond_fpe: tuple[float, float, float],
    loss_param_name: str,
    alpha_key: str ='alpha',
    funs: list = None,
    ...
    initial_guess_reg_and_loss_param : tuple (lambda_guess, loss_param_guess)
    ...
    def minimize_fun(reg_and_loss_param):
        reg_param = float(reg_and_loss_param[0])
        loss_param = float(reg_and_loss_param[1])

        if verbose:
            print(f"\t\tλ = {reg_param:.5f}, {loss_param_name} = {loss_param:.5f}")
        if reg_param < min_reg_param or loss_param < min_loss_param:
            if verbose:
                print(f"\t\tλ < min_reg_param ({min_reg_param}) or {loss_param_name} < min_loss_param ({min_loss_param}), returning barrier penalty")
            return barrier_penalty + 1e3 * (reg_param - min_reg_param)**2 + 1e3 * (loss_param - min_loss_param)**2
        ...
        if barrier_V_threshold is not None and (V >= barrier_V_threshold or V < 0):
            if verbose:
                print(f"\t\tBarrier on V violated: V = {V}, threshold = {barrier_V_threshold} at λ = {reg_param:.5f}, {loss_param_name} = {loss_param:.5f}")
            return barrier_penalty
        ...
        if RS_func is not None and barrier_RS_threshold is not None:
            ...
            rs_kwargs.update(local_f_hat_kwargs)
            rs_kwargs["reg_param"] = reg_param
            rs_kwargs[loss_param_name] = loss_param
            # Extract alpha
            alpha_val = float(local_f_hat_kwargs.get(alpha_key, 1.0))
            rs_kwargs[alpha_key] = alpha_val
            try:
                ...
                if RS_val >= barrier_RS_threshold:
                    if verbose:
                        print(f"\t\tBarrier on RS violated: RS = {RS_val}, threshold = {barrier_RS_threshold} at λ = {reg_param:.5f}, {loss_param_name} = {loss_param:.5f}, alpha = {alpha_val}")
                    return barrier_penalty + 1e3 * (RS_val - barrier_RS_threshold)**2
            except Exception as e:
                print(f"\t\tRS_func raised an exception: {e}")
                return barrier_penalty
    ...
    x0 = [float(initial_guess_reg_and_loss_param[0]), float(initial_guess_reg_and_loss_param[1])]
    ...
    if not result.success:
        raise MinimizationError("find_optimal_reg_and_loss_param_function", initial_guess_reg_and_loss_param)
