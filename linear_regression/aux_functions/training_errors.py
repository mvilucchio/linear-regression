...
def training_error_Hinge_loss_probit(m: float, q: float, Σ: float, delta: float) -> float:
    ...
    for y_val, domain in domains:
        int_value += quad(integral_training_error_Hinge_probit, *domain, args=(y_val, m, q, Σ, delta))[0]
...
def training_error_Logistic_loss_probit(m, q, Σ, delta):
    ...
    int_value = 0.0
    for y_val, domain in domains:
        int_value += quad(integral_training_error_Logistic_probit, *domain, args=(y_val, m, q, Σ, delta))[0]
...
def training_error_Exponential_loss_probit(m, q, Σ, delta):
    ...
    int_value = 0.0
    for y_val, domain in domains:
        int_value += quad(integral_training_error_Exponential_probit, *domain, args=(y_val, m, q, Σ, delta))[0]
...
def training_error_Hinge_loss_no_noise(m, q, Σ):
    ...
    int_value = 0.0
    for y_val, domain in domains:
        int_value += quad(integral_training_error_Hinge_no_noise, domain[0], domain[1], args=(y_val, m, q, Σ))[0]
...
def integral_training_error_Logistic_no_noise(ξ, y, m, q, Σ):
    η = m**2 / q
    proximal = minimize_scalar(moreau_loss_Logistic, args=(y, sqrt(q) * ξ, Σ))["x"]
    return 0.5 * (gaussian(ξ, 0, 1) * (1 + erf(y * sqrt(0.5 * η / (1 - η)) * ξ)) * logistic_loss(y, proximal))
...
def training_error_Logistic_loss_no_noise(m, q, Σ):
    ...
    int_value = 0.0
    for y_val, domain in domains:
        int_value += quad(integral_training_error_Logistic_no_noise, *domain, args=(y_val, m, q, Σ))[0]
...
def training_error_Exponential_loss_no_noise(m, q, Σ):
    ...
    int_value = 0.0
    for y_val, domain in domains:
        int_value += quad(integral_training_error_Exponential_no_noise, *domain, args=(y_val, m, q, Σ))[0]
